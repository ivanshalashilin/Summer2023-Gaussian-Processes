...
#FilLoc= ''
FilLoc= 'HelmholtzGP/'
Pos_Train =onp.genfromtxt(FilLoc+'data/XY_train_vortex.csv', delimiter=',').T
Pos_Test =onp.genfromtxt(FilLoc+'data/XY_test_vortex.csv', delimiter=',').T
Vel_Train =onp.genfromtxt(FilLoc+'data/UV_train_vortex.csv', delimiter=',').T
Vel_Test =onp.genfromtxt(FilLoc+'data/UV_test_vortex.csv', delimiter=',').T
shape = (17,17)


fig, ax = plt.subplots(1,1)
ax.quiver(Pos_Test[0], Pos_Test[1], Vel_Test[0], Vel_Test[1], label = '$D_0$', scale = 20)
ax.quiver(Pos_Train[0],Pos_Train[1], Vel_Train[0], Vel_Train[1], color = 'red', alpha = 0.7, label = '$D_T$', scale = 20)
...
PosTest3D = jnp.vstack((jnp.repeat(Pos_Test.reshape(2,NTest), repeats=2 , axis = 1), zLabelTest)).T
...
Likelihood = gpx.Gaussian(num_datapoints=D0.n, obs_noise= jnp.array([1.0],dtype=jnp.float64))
...
Vel_Lat = [VelocityMean[::2], VelocityMean[1::2]]
Pos_Lat = Pos_Test
...
ResidualsVel = jnp.sqrt((Vel_Test[0]-Vel_Lat[0])**2+(Vel_Test[1]-Vel_Lat[1])**2)
X = Pos_Test[0]
Y = Pos_Test[1]

#make figure
fig, ax = plt.subplots(1,3 , figsize = (17.5, 3.2))
fig.tight_layout()
#ground truth
ax[0].quiver(Pos_Train[0],Pos_Train[1], Vel_Train[0], Vel_Train[1], color = 'red', label = 'Testing Data')
ax[0].quiver(Pos_Test[0], Pos_Test[1], Vel_Test[0], Vel_Test[1], label = 'Training data')
ax[0].set( aspect = 'equal', title = 'Ground Truth')

#Latent estimate of vector field F
ax[1].quiver(Pos_Lat[0],Pos_Lat[1], Vel_Lat[0], Vel_Lat[1], color = 'darkblue', label = 'Latent estimate of $\mathbf{F}$')
ax[1].quiver(Pos_Train[0], Pos_Train[1], Vel_Train[0], Vel_Train[1], color = 'red')
ax[1].set(aspect = 'equal', title = 'GP Latent Estimate')

im = ax[2].imshow(ResidualsVel.reshape(shape),  extent=[X.min(), X.max(), Y.min(), Y.max()], cmap = 'hot')
ax[2].set(aspect = 'equal', title = 'Residuals')
plt.colorbar(im,orientation ='vertical', fraction=0.03, pad=0.04)
plt.savefig('gulfvelocity.pdf')
#plt.show()
...
class HelmholtzKernel(gpx.kernels.AbstractKernel):
    ...
    def __call__(
        ...
        DivPsi =  -jnp.array(hessian(self.kernelPsi)(X,Xp),dtype = jnp.float64)[1-z][1-zp]
...
Helm_Lat = [HelmholtzMean[::2], HelmholtzMean[1::2]]
Pos_Lat = Pos_Test
...
#Residuals between ground truth and estimate
ResidualsHelm = jnp.sqrt((Vel_Test[0]-Helm_Lat[0])**2+(Vel_Test[1]-Helm_Lat[1])**2)
X = Pos_Test[0]
Y = Pos_Test[1]
...
ax[0].set(aspect = 'equal', title = 'Ground Truth')
#Latent estimate of vector field F
ax[1].quiver(Pos_Lat[0],Pos_Lat[1], Helm_Lat[0], Helm_Lat[1], color = 'darkblue', label = 'Latent estimate of $\mathbf{F}$')
ax[1].quiver(Pos_Train[0], Pos_Train[1], Vel_Train[0], Vel_Train[1], color = 'red')
ax[1].set(aspect = 'equal', title = 'GP Latent Estimate of $\mathbf{F}$')
#Residuals
im = ax[2].imshow(ResidualsHelm.reshape(shape),  extent=[X.min(), X.max(), Y.min(), Y.max()], cmap = 'hot')
ax[2].set(aspect = 'equal', title = 'Residuals')
plt.colorbar(im,orientation ='vertical', fraction=0.03, pad=0.04)
plt.savefig('gulfhelmholtz.pdf')
plt.show()
...
# %% [markdown]
# The Helmholtz model significantly outperforms the velocity model in this toy example, as indicated by the lower NLPD score.
...
shape = (int(gulf_data['shape'][0]), int(gulf_data['shape'][1]))

#split 80-20 training testing
from sklearn.model_selection import train_test_split

Pos_Test, Pos_Train, Vel_Test, Vel_Train = train_test_split(Pos,Vel, test_size=0.03, random_state=41)
